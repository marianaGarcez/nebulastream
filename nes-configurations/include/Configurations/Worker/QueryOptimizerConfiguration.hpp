...
public:
    ...
    QueryOptimizerConfiguration(const std::string& name, const std::string& description)
        : BaseConfiguration(name, description)
    {
    }

    NES::EnumOption<DumpMode> dumpMode
        = {"dumpMode", DumpMode::NONE, "If and where to dump query compilation details [NONE|CONSOLE|FILE|FILE_AND_CONSOLE]."};
    NES::StringOption dumpPath = {"dumpPath", "", "Path to dump query compilation details."};

    NES::EnumOption<NES::ExecutionMode> executionMode
        = {"executionMode",
           NES::ExecutionMode::COMPILER,
           "ExecutionMode for the query compiler [COMPILER|INTERPRETER]."};

    NES::UIntOption numberOfPartitions
        = {"numberOfPartitions",
           std::to_string(DEFAULT_NUMBER_OF_PARTITIONS_DATASTRUCTURES),
           "Partitions in a hash table",
           {std::make_shared<NES::NumberValidation>()}};

    NES::UIntOption pageSize
        = {"pageSize",
           std::to_string(DEFAULT_PAGED_VECTOR_SIZE),
           "Page size of any other paged data structure",
           {std::make_shared<NES::NumberValidation>()}};

    NES::UIntOption operatorBufferSize
        = {"operatorBufferSize",
           std::to_string(DEFAULT_OPERATOR_BUFFER_SIZE),
           "Buffer size of an operator e.g. during scan",
           {std::make_shared<NES::NumberValidation>()}};

    NES::EnumOption<StreamJoinStrategy> joinStrategy
        = {"joinStrategy",
           StreamJoinStrategy::NESTED_LOOP_JOIN,
           "WindowingStrategy [HASH_JOIN_LOCAL|HASH_JOIN_GLOBAL_LOCKING|HASH_JOIN_GLOBAL_LOCK_FREE|NESTED_LOOP_JOIN]."};

    NES::StringOption pipelinesTxtFilePath = {"pipelinesTxtFilePath", "pipelines.txt", "Path to dump pipeline details."};
