...
/// `circle_circumference(Circle) -> Float64`. Calculates the circumference area of a circle.
class CircleCircumferenceLogicalFunction final
{
public:
    static constexpr std::string_view NAME = "CIRCLE_CIRCUMFERENCE";

    explicit CircleCircumferenceLogicalFunction(const LogicalFunction& child);

    [[nodiscard]] bool operator==(const CircleCircumferenceLogicalFunction& rhs) const;
...
private:
    ...
    friend Reflector<CircleCircumferenceLogicalFunction>;
};
...
{
struct ReflectedCircleCircumferenceLogicalFunction
{
    std::optional<LogicalFunction> child;
};
}

FMT_OSTREAM(NES::CircleCircumferenceLogicalFunction);
