...
/// `to_celsius(ThermalPixel) -> FLOAT32`. Nominal type-gating: rejects
/// arguments whose runtime DataType isn't STRUCT named "ThermalPixel".
class ToCelsiusLogicalFunction final
{
public:
    static constexpr std::string_view NAME = "TO_CELSIUS";

    explicit ToCelsiusLogicalFunction(const LogicalFunction& child);

    [[nodiscard]] bool operator==(const ToCelsiusLogicalFunction& rhs) const;
